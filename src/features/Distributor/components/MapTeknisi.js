...
export default function MapTeknisi({id}) {
  ...
  async function handleMapDataTeknisi() {
    try {
      const response = await api.get('/distributor/teknisi');
      // console.log('data teknisi', response.data);
      setDataTeknisi(response.data);
    } catch (error) {
      if (error.response) {
