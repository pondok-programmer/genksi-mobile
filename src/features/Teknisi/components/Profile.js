    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.data.message === 'Unauthenticated.') {
          // Jika pesan dari server adalah "Unauthenticated."
          // maka arahkan ke layar login
          navigation.replace('Login');
        } else {
          // Penanganan kesalahan lainnya
          console.log('Error from server:', error.response.data.message);
          // Tambahkan pesan error ke pengguna jika diperlukan
        }
      } else {
        console.log('Error:', error.message);
        // Tindakan jika terdapat kesalahan yang bukan dari server
      }
    }
  }
